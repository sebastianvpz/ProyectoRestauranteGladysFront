...
export function Footer() {
  return (
    <footer className="bg-[#2D2013] py-16 px-6">
      <Container size="lg">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-[#C75D3A] flex items-center justify-center">
              <span className="text-white font-display font-bold text-2xl">G</span>
            </div>
            <span className="font-display text-3xl font-semibold text-[#FDF8F3]">
...
function SocialIcon({ href, path }: { href: string; path: string }) {
  return (
    <a
      href={href}
      className="w-10 h-10 bg-[#FDF8F3]/10 rounded-full flex items-center justify-center hover:bg-[#C75D3A] transition-colors"
    >
      <svg className="w-5 h-5 text-[#FDF8F3]" fill="currentColor" viewBox="0 0 24 24">
        <path d={path} />
      </svg>
    </a>
  );
